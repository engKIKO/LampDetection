            ...
            for lamp_name, (x1, y1, x2, y2) in lamp_zones.items():
                ...
                if 'bounding_boxes' in results['result']:
                    for box in results['result']['bounding_boxes']:
                        ...
                        detection_buffer.add_detection(label,confidence)
                            ...
                            if label.lower() == "on":
                                if time_is_night():  # if time is night
                                    if mqttHandel.LDR_is_night() == True:
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="runnig",
                                                                   confidence=confidence,
                                                                   reason="Normally lamp running", ldr_value=ldrValues)
                                        detection_buffer.clear()
                                    else:

                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="runnig",
                                                                   confidence=confidence,
                                                                   reason=f"A Time is night {tempTime} not releate with  LDR is Days {ldrValues} Lamp is running",
                                                                   ldr_value=ldrValues, level="Err")
                                        detection_buffer.clear()
                                    ...
                                    if mqttHandel.LDR_is_night() == True:
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="runnig",
                                                                   confidence=confidence,
                                                                   reason=f"B Time is days {tempTime} not releate with LDR is Night {ldrValues} Lamp is running",
                                                                   ldr_value=ldrValues, level="Err")
                                        detection_buffer.clear()
                                    else:
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="runnig",
                                                                   confidence=confidence,
                                                                   reason=f"C Time is days {tempTime}  releate with LDR is Days {ldrValues} but Lamp shuould not running",
                                                                   ldr_value=ldrValues, level="Warn")
                                        detection_buffer.clear()
                                ...
                                if time_is_night():
                                    if mqttHandel.LDR_is_night() == True:
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="broken",
                                                                   confidence=confidence,
                                                                   reason=f"D Time is Night {tempTime}  releate with LDR is Night{ldrValues} but Lamp si not running",
                                                                   ldr_value=ldrValues, level="Warn")
                                        detection_buffer.clear()
                                    else:
                                        ...
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="broken",
                                                                   confidence=confidence,
                                                                   reason=f"E Time is Night {tempTime} not releate with LDR is Days{ldrValues} but Lamp is should running",
                                                                   ldr_value=ldrValues, level="Err")
                                        detection_buffer.clear()
                                else:
                                    if mqttHandel.LDR_is_night() == True:
                                        ...
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="broken",
                                                                   confidence=confidence,
                                                                   reason=f"F Time is Days {tempTime} not releate with LDR is Night{ldrValues} but Lamp is should running",
                                                                   ldr_value=ldrValues, level="Err")
                                        detection_buffer.clear()
                                    else:
                                        ...
                                        lampLogger.log_lamp_status(lamp_id=lamp_name, status="broken",
                                                                   confidence=confidence,
                                                                   reason=f"G Time is Days {tempTime}  releate with LDR is Days {ldrValues} Lamp is not running",
                                                                   ldr_value=ldrValues, level="Info")
                                        detection_buffer.clear()

                            else:

                                lampLogger.log_lamp_status(lamp_id=lamp_name, status="unknow", confidence=confidence,
                                                           reason=f"H Not detection Time is Days {tempTime}  releate with LDR is Days {ldrValues}",
                                                           ldr_value=ldrValues, level="Err")
                                detection_buffer.clear()
